"""Validation utilities for Monte Carlo simulations."""
...
def validate_rate_paths(
    ...
    *,
    ...
    """Basic sanity checks for generated rate paths."""
    failed: list[str] = []
    warnings: list[str] = []
    ...
    if np.any(np.isnan(stacked)) or np.any(np.isinf(stacked)):
        failed.append("nan_values")
    if float(stacked.max()) > 0.20:
        warnings.append("rates_exceed_20pct")
    if float(stacked.min()) < 0.0:
...
def validate_distribution(
    ...
    *,
    ...
    """Validate ordering and range of PV distribution."""
    failed: list[str] = []
    warnings: list[str] = []
    ...
    if mean != 0 and std / abs(mean) > 0.20:
